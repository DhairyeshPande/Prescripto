...
const AddDoctor = () => {
  ...
  const onSubmitHandler=async(event)=>{
    ...
    try {
      ...
      //api calling to backend to save form data
      const {data}=await axios.post(backendUrl + '/api/admin/add-doctor', formData, {headers:{aToken} })
      
      if(data.success){
        toast.success(data.message)
        //ek baar submit krne ke baad firse empty krna h, taki new doctor krte samay vo filled naa dikhe
        setDocImg(false)
        setName('')
        setEmail('')
        setPassword('')
      ...
      <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
        ...
        <div className='flex flex-col lg:flex-row items-start gap-10 text-gray-600'>
          <div className='w-full lg:flex-1 flex flex-col gap-4'>
            ...
            <div className='flex-1 flex flex-col gap-1'>
              <p>Fees</p>
              {/* <input onChange={(e)=>setFees(Number(e.target.value))} value={fees} className='border rounded px-3 py-2' type="number" placeholder='Fees' required/> */}
