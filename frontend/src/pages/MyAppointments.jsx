...
const MyAppointments = () => {
  ...
  // date vo dislpay ho rha, appointment page pe vo date_month_year, aisa dikha rha, usko change kr rhe
  ...
  const getUserAppointments=async()=>{
    ...
    try {
      ...
      // making api call to get appointments data
      const {data}=await axios.get(backendUrl + '/api/user/appointments', {headers:{token}})

      if(data.success){
        setAppointments(data.appointments.reverse()) // we want latest appointment on top, so using reverse function
  ...
  const cancelAppointment=async(appointmentId)=>{
    ...
    try {
      ...
      if(data.success){
        ...
        //updating users appointment data
        getUserAppointments()
        getDoctorsData() // appointment cancel krne ke baad vo slot doctor ke page pe firse dikhna chahiye, without reloading
        } else {
  ...
  //       if(data.success){
  //         //console.log(data.order) // data ka structure mil jata, fir vaha se jo property chahiye vo le lete
  //         initPay(data.order)
  //       }
      ...
      <div>
        {appointments.map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            <div>
              {/* <img className='w-32 bg-indigo-50' src={item.image} alt="doctor" /> error aa rha,
              undefined wala, console.og se dekha ki doc ki mage docdata.image krke mil jayegi */ }
            ...
            <div className='flex-1 text:sm text-zinc-600'>
              ...
              {/* Changing slot date format <p className='text-sm mt-1'><span className='text-sm text-neutral-700 font-medium'>Date & Time:</span> {item.slotDate} | {item.slotTime }</p> */}
            ...
            <div className='flex flex-col gap-2 justify end'>
              ...
              {item.cancelled && !item.isCompleted && <button className='sm:min-w-48 py-2 border border-red-600 rounded text-red-500'>Appointment Cancelled</button>}
