...
const Appointment = () => {
  ...
  const getAvailableSlots=async()=>{
    ...
    for(let i=0;i<7;i++){
      ...
      while(currentDate<endTime){
        ...
        // slots book krne ke baad vo slot nhi dikhna chahiye user ko
        let day=currentDate.getDate()
        let month=currentDate.getMonth()+1 // +1 kyuki indexing 0 se start hoti
        let year=currentDate.getFullYear()

        const slotDate=day+"_"+month+"_"+year
        const slotTime=formattedTime

        // checking whether the selctted slot is booked by someone else or not
        //if booked, then we will not push in timeSlots.push(niche)
  ...
  //for booking apppointment-> api
  const bookAppointmnet=async()=>{
    ...
    try {
      ...
      const date=docSlot[slotIndex][0].datetime //by clciking on date, arr-0..6

      let day=date.getDate()
      let month=date.getMonth() //jan==0, feb=1,...
